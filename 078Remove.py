...
class DoublyLinkedList:
    ...
    def insert(self, index, value):
        # Check index<0 or >length
        if index < 0 or index > self.length:
            return None
        # Check index 0->prepend length-1->append
        if index == 0:
            print("Prepend method Applied :")
            return self.prepend(value)
        if index == self.length:
            print("Append method Applied :")
            return self.append(value)
        # assign new .node
        new_node = Node(value)
        # assign before after Nodes
        before = self.get(index-1)
        after = before.next
        # links new Node
        new_node.prev = before
        new_node.next = after
        # links before and after nodes
    ...
    def remove(self, index):
        ...
        if index < 0 or index > self.length:
            return None
        # check index if=0 pop first if length-1 pop
        if index == 0:
            print("First Pop method applied")
            return self.pop_first()
        if index == self.length-1:
            print("Pop method applied")
            return self.pop()
        # temp get(index-1) before=temp.prev after=temp.next
        temp = self.get(index)
        temp.next.prev = temp.prev
        temp.prev.next = temp.next
        # make temp.prev and temp.next None
        temp.prev = None
        temp.next = None

        # dec length and return temp
        self.length -= 1
        return temp
...
index = 4
if index < 0 or index > my_dll.length-1:
    print(f"{index} out of range of length of Link List")
else:

    print(f"\n The Removed ({index}, {my_dll.remove(index).value})")
    my_dll.print_list()
